...
void MinTest::runAll(void) {
    testStart("simple min_3_1 test");
    timer_start();
    get_test_quant_min_3_1_ctx(ctx);
    S_TENSOR min_x = ctx.get("min_x_3_1:0");
    ctx.eval();
    timer_stop();

    Tensor* ref_min = t_import.float_import("/fs/idx_data/output_min_x3_1.idx");
