def test_linear_reorder_1(subj_ugraph_1):
    ...
    for op in new_ugraph['output'].input_nodes:
        assert op.op_type == 'ReLUOperator', 'expecting ReLUOperator, get {}'.format(op.op_type)
        pool_op = op.input_nodes[0]
        assert pool_op.op_type == 'MaxPoolOperator', \
            'expecting MaxPoolOperator as input of Relu, get {}'.format(pool_op.op_type)
