...
{% endif %}
{
    ...
    ctx.add(new RamTensor<{{out_dtypes[0]}}>({1}, {{address[1]}}), "{{outputs[1]}}", {{ref_counts[1]}});
    ctx.add(new RamTensor<{{out_dtypes[1]}}>({1}, {{address[2]}}), "{{outputs[2]}}", {{ref_counts[2]}});
    {%else%}
    ctx.add(new RamTensor<{{qout_dtype}}>({{address[0]}}), "{{outputs[0]}}");
    ctx.add(new RamTensor<{{out_dtypes[0]}}>({1}, {{address[1]}}), "{{outputs[1]}}");
    ctx.add(new RamTensor<{{out_dtypes[1]}}>({1}, {{address[2]}}), "{{outputs[2]}}");
    {%endif%}
