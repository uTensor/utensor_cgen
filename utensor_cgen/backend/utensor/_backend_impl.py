...
class uTensorBackend(Backend):
  ...
  def __init__(self, config, code_generator=None, graph_lower=None):
    ...
    if code_generator is None:
      part_config = final_config['backend'][uTensorCodeGenerator.PART]
      code_generator = uTensorCodeGenerator(config=part_config)
    if graph_lower is None:
      part_config = final_config['backend'][uTensorGraphLower.PART]
  ...
  def default_config(cls):
    config = {}
    config[cls.TARGET] = {}
    config[cls.TARGET]['backend'] = {}
    config[cls.TARGET]['backend'][uTensorCodeGenerator.PART] = uTensorCodeGenerator.default_config
    config[cls.TARGET]['backend'][uTensorGraphLower.PART] = uTensorGraphLower.default_config
    return config
